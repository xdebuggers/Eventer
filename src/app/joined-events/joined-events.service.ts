    ...
    return this.loginService.userId.pipe(
      ...
      switchMap(token => {
        ...
        return this.http.post<{ name: string }>(
          this.fireBaseURL + `/joined-events.json?auth=${token}`,
          { ...newJoined, id: null }
        );
      }),
      switchMap(resData => {
    ...
    return this.loginService.token.pipe(
      ...
      switchMap(token => {
        return this.http.delete(
          this.fireBaseURL +
            `/joined-events/${joinedEventId}.json?auth=${token}`
        );
      }),
      switchMap(() => {
        return this.joinedEvents;
    ...
    return this.loginService.userId.pipe(
      ...
      switchMap(token => {
        return this.http.get<{ [key: string]: JoinedEventData }>(
          this.fireBaseURL +
            `/joined-events.json?orderBy="userId"&equalTo="${fetchedUserId}"&auth=${token}`
        );
      }),
      map(joinedEventData => {
        const joinedEvents = [];
  ...
  fetchJoinedEventComments(eventId: string) {
    ...
    return this.loginService.userId.pipe(
      ...
      switchMap(token => {
        return this.http.get<{ [key: string]: JoinedEventData }>(
          this.fireBaseURL +
            `/joined-events.json?orderBy="eventId"&equalTo="${eventId}"&auth=${token}`
        );
      }),
      map(joinedEventData => {
        ...
        for (const key in joinedEventData) {
          if (joinedEventData.hasOwnProperty(key)) {
            joinedEventComments.push(
              new JoinedEventComments(
                joinedEventData[key].type,
                joinedEventData[key].firsName,
                joinedEventData[key].lastName,
                joinedEventData[key].comment
